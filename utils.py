...
def pad_sents_char(sents, char_pad_token):
    ...
    max_word=21
    sents_padded = []
    pad_char=[char_pad_token]*max_word
    for sent in sents:
        sent_padded=[]
        for char in sent:
            char_padded =char[: max_word]
            while len(char_padded) < max_word:
                char_padded.append(char_pad_token)
            sent_padded.append(char_padded)

        while len(sent_padded) < max_sent:
            sent_padded.append(pad_char[:])

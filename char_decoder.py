...
class CharDecoder(nn.Module):
    def __init__(self, hidden_size, char_embedding_size=50, target_vocab=None):
        ...
        Vsize = len(target_vocab.char2id)
        self.charDecoder = nn.LSTM(char_embedding_size, hidden_size)
        self.char_output_projection = nn.Linear(hidden_size, Vsize)
        self.decoderCharEmb = nn.Embedding(Vsize, char_embedding_size, padding_idx=target_vocab.char2id['<pad>'])
    ...
    def forward(self, input, dec_hidden=None):
        ...
        charembed = self.decoderCharEmb(input)
        h_t, dec_hidden = self.charDecoder(charembed, dec_hidden)
    ...
    def train_forward(self, char_sequence, dec_hidden=None):
        ...
        cross_entropy_loss_layer = nn.CrossEntropyLoss(ignore_index=self.padding_idx, reduction="sum")
        loss = cross_entropy_loss_layer(s_t.permute(1, 2, 0), char_sequence[1:].permute(1,0))
        return loss
    ...
    def decode_greedy(self, initialStates, device, max_length=21):
        ...
        t=0
        while t <= max_length - 1:
            s_t, dec_hidden = self.forward(current_char_tensor, dec_hidden)
            current_char_tensor = s_t.argmax(-1)
